...
打开 [coffee.hexly.ai](https://coffee.hexly.ai)，在顶部切换语言和主题，再探索风味轮、产区、冲煮、学堂、手记与展示模式。
...
部署配置、授权与回滚见[部署说明](docs/deployment.md)。

## 测试

`bun run test` 检查内容完整性、配方计算、搜索、推荐与存储行为。首次运行浏览器测试：
...
- [架构](docs/architecture.md)与[内容模型](docs/content-model.md)。
- [部署说明](docs/deployment.md)与[研究记录](docs/research-sources.md)。
